...
from guizero import App, Text, TextBox, MenuBar
...
def main_bg():
    bg_value = app.question("Background", "Enter Hex Color Value.")
    if bg_value is not None:
        app.bg = bg_value


def text_clr():
    text_value = app.question("Text", "Enter Hex Color Value.")
    if text_value is not None:
        app.text_color = text_value
...
class GuiSetup:
    ...
    def read_file(self, file):
        """Read telegram JSON config file"""
        read = False
        while read is False:
            try:
                with open(
    ...
    def update_textbox(self, txtbox, file, key) -> None:
        ...
        last_price = txtbox.value if txtbox.value != "" else value
        txtbox.value = value if not value == " " else "N/A"
    ...
    def update_indicator(self, txtbox, file, key) -> None:
        ...
        txtbox.value = indicators.get(key) if key in indicators.keys() else False
    ...
    def update_lastaction(self, txtbox, file) -> None:
        ...
        txtbox.value = "BUY" if not margin == " " else "SELL"
        txtbox.bg = "#38BC38" if not margin == " " else app.bg
